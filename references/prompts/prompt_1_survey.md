...
7. **补充信息**（自由文本，以下为提示项）：
   ...
   - 视觉风格偏好（如公司有品牌规范）
   - **AI 配图偏好**：
     - A. 不需要配图（推荐，生成速度最快，~25 分钟完成）
     - B. 只在关键页面配图（封面 + 章节封面，约 3-5 张。需配置 .env 中的 IMAGE_API_KEY，生成时间增加 15-20 分钟，受 API 稳定性影响部分图片可能失败）
     - C. 每页都配图（氛围感最强，生成时间最长约 45+ 分钟，需配置 IMAGE_API_KEY）
     - D. 用户提供图片素材（请提供图片路径）
